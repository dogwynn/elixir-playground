defprotocol DNS.PacketEncoder do
  @moduledoc """
  Given some struct data, this protocol is responsible for encoding
  that information as network-ready bitstrings
  """
...
defprotocol DNS.RR.Atoms do
  @doc "What is the resource record's class as an atom"
  @spec class(map) :: :IN | :CH | :unhandled
  def class(rr)

  @doc "What is the resource record's type as an atom"
  @spec type(map) :: :atom
  def type(rr)
end
...
defmodule DNS.RR do
  ...
  import DNS, only: [
    encode_name: 1, decode_name: 2, data_after_name: 1,
    rr_buffer: 1, rr_fields: 1, rr_answer_fields: 1,
  ...
  def raw_name(buffer, name \\ "") do
    ...
    case buffer do
      ...
      << 0::1, 0::1, size :: 6, buffer :: binary >> ->
        # IO.inspect(size)
        raw_name(
          bin_slice(buffer, size),
          name <> << 0::1, 0::1, size :: 6 >> <> bin_slice(buffer, 0, size)
        )
      _ -> {:error, :bad_rr_name_encoding}
    end
  end

  defimpl DNS.PacketEncoder do
    @spec to_bitstring(DNS.RR.t) :: binary
    def to_bitstring(rr = %DNS.RR{}) do
      with {:ok, name} <- encode_name(rr.name) do
        name <> <<
        rr.type :: 16,
          bool_int(rr.flush) :: 1,
          rr.class :: 15,
  ...
  def from_bitstring(packet, buffer) do
    with {:ok, name} <- decode_name(packet, buffer),
         {:ok, rr_rest} <- data_after_name(buffer),
         {:ok, {fields, answer_rest}} <- rr_fields(rr_rest),
         {:ok, {answer_fields, rest}} <- rr_answer_fields(answer_rest)
      do
      {:ok, {
          %DNS.RR{name: name}
          |> Map.merge(fields)
  ...
  defimpl DNS.RR.Atoms do
    defdelegate class(query), to: DNS.RR
    defdelegate type(query), to: DNS.RR
  end
end
...
defmodule DNS.Query do
  ...
  def from_bitstring(packet, buffer) do
    with {:ok, name} <- decode_name(packet, buffer),
         {:ok, rr_rest} <- data_after_name(buffer),
         {:ok, {fields, rest}} <- rr_fields(rr_rest) do
      {:ok, {
          %DNS.Query{name: name}
...
defmodule DNS.Packet do
  ...
  defimpl DNS.PacketEncoder do
    ...
    def to_bitstring(packet = %DNS.Packet{}) do
      ...
      >> <> (
        packet.queries
        |> Enum.map(&DNS.PacketEncoder.to_bitstring/1)
        |> Enum.join("")
      ) <> (
        packet.answers
        |> Enum.map(&DNS.PacketEncoder.to_bitstring/1)
